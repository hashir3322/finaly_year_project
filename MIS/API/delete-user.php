<?php 
	...
	$sql = "SELECT *FROM registered_user_tbl WHERE user_id = '{$userId}'";
	$result = mysqli_query($conn,$sql) or die (mysqli_error($conn));

	if(mysqli_num_rows($result) > 0){
		$deleteSql = "DELETE FROM registered_user_tbl WHERE user_id = '{$userId}'";
		if(mysqli_query($conn,$deleteSql)){
			echo json_encode(array('message'  => 'User Deleted Successfully.', 'status' => True));
		}else{
			echo json_encode(array('message'  => 'Could not delete User', 'status' => False));
		}
	}else{
